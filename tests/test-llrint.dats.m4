...
main() =
  let
    m4_foreachq([t],[all_floattypes],      
      [#if HAVE_floattypes_llrint_[]t #then
       macdef f2f = $UN.cast{t}
       val _ = $extfcall(int, "fesetround", FE_TONEAREST)
       val _ = assertloc(llrint(f2f(~4.8)) = ~5LL)
       val _ = assertloc(llrint(f2f(~4.2)) = ~4LL)
       val _ = assertloc(llrint(f2f(4.2)) = 4LL)
       val _ = assertloc(llrint(f2f(4.8)) = 5LL)
       val _ = $extfcall(int, "fesetround", FE_UPWARD)
       val _ = assertloc(llrint(f2f(~4.8)) = ~4LL)
       val _ = assertloc(llrint(f2f(~4.2)) = ~4LL)
       val _ = assertloc(llrint(f2f(4.2)) = 5LL)
       val _ = assertloc(llrint(f2f(4.8)) = 5LL)
